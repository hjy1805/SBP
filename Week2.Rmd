...
```{r ACTIVITY 1}
# Assuming topTable1 and topTable2 are the two top tables
common_genes <- intersect(topTable(fit, coef=ncol(design))$Gene, topTable(fit, coef=ncol(design))$Gene)
unique_genes_in_table1 <- setdiff(topTable(fit, coef=ncol(design))$Gene, topTable(fit, coef=ncol(design))$Gene)
unique_genes_in_table2 <- setdiff(topTable(fit, coef=ncol(design))$Gene, topTable(fit, coef=ncol(design))$Gene)
...
# Store all of them
table1 <- topTable(fit2) 
table2 <- topTable(fit2,coef=1) 
table3 <-topTable(fit2,coef=2) 
table4 <-topTable(fit2,coef=3) 
...
gsea_gene_sets <- unique(gsea_results$GeneSet)
ora_gene_sets <- unique(ora_results$GeneSet)

# Create a Venn diagram
venn.plot <- venn.diagram(
  x = list(GSEA = gsea_gene_sets, ORA = ora_gene_sets),
  category.names = c("GSEA", "ORA"),
  filename = NULL,
  output = TRUE
)
